...
async def entrypoint(ctx: JobContext):
    ...
    if not presentation_id:
        logger.error("❌ No presentation_id found.")
        return
    ...
    # ---------------------------------------------------------
    current_index = -1 # Start before the first slide
    
    async def show_slide(index):
        """Helper to update frontend and speak slide content"""
        slide = slides[index]
        image_url = slide.get("image_url", "")
        content_text = slide.get("extracted_text", "")

        # Update Slide on Screen
        await ctx.room.local_participant.set_attributes({"current_slide_url": image_url})
        
        # AI Presents Slide
        instruction = f"Present this slide: {content_text}. Be brief (1-2 sentences)."
        handle = session.generate_reply(instructions=instruction)
        await handle.wait_for_next_playout()

    @session.on("user_speech_committed")
    def on_user_speech(msg):
        """Listens for 'Next' or 'Previous' commands"""
        ...
        if "next" in text or "forward" in text:
            if current_index < len(slides) - 1:
                current_index += 1
                asyncio.create_task(show_slide(current_index))
    ...
    await session.start(
        agent=Agent(instructions=f"{SYSTEM_INSTRUCTIONS}\nYou respond to 'Next' and 'Previous' commands."),
        room=ctx.room,
        room_input_options=room_io.RoomInputOptions(video_enabled=True),
    )

    # Initial Welcome
    session.generate_reply(instructions="Welcome the user. Tell them to say 'Next' whenever they are ready to start.")
    
    logger.info("✅ Agent ready for voice commands.")
    await keep_alive(ctx)
